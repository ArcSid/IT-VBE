...
void skaitytiIsFailo(vector<Parduotuve> &parduotuves, float &piniguKiekis, vector<string> &prekes)
{
...
void rez(float piniguKiekis, float maziausiaKaina, vector<Parduotuve> parduotuves)
{
    ofstream rez("U1rez.txt");
    if (maziausiaKaina > piniguKiekis)
    {
    ...
    for (int i = 0; i < parduotuves.size(); i++)
    {
        if (parduotuves[i].prekes.size() > 0)
        {
            ...
            {
                rez << left << setw(10) << parduotuves[i].prekes[j] << " ";
            }
            rez << endl;
        }
    }
